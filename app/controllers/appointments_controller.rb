class AppointmentsController < ApplicationController
    ...
    def index 
      @patient = current_receptionist.patients.find_by_id(params[:patient_id])
      @doctor = Doctor.find_by_id(params[:doctor_id])
      if @patient
        @appointments = @patient.appointments
      elsif @doctor 

        @appointments = current_receptionist.appointments.by_doctor(@doctor)
      else
        @appointments = current_receptionist.appointments
      end
      filter_options
    end
    ...
    def set_appointment 
      @appointment = current_receptionist.appointments.find(params[:id])
    end
